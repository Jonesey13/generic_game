...
impl CollisionTestBuilder {
    ...
    pub fn add_poly<'a> (&'a mut self, con_poly: ConPoly) -> &'a mut Self {
        let con_poly_wrapper = CollObjectWrapper::new(
            con_poly,
            self.object_index,
    ...
    pub fn add_circle<'a> (&'a mut self, circle: Circle) -> &'a mut Self {
        let circle_wrapper = CollObjectWrapper::new(
            circle,
            self.object_index,
    ...
    pub fn add_point<'a> (&'a mut self, point: Point) -> &'a mut Self {
        let point_wrapper = CollObjectWrapper::new(
            point,
            self.object_index,
    ...
    pub fn add_line<'a> (&'a mut self, line: Line) -> &'a mut Self {
        let line_wrapper = CollObjectWrapper::new(
            line,
            self.object_index,
